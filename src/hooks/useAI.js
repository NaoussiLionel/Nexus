  ...
  const sendChatMessage = useCallback(async (userText) => {
    ...
    function buildSystemPrompt() {
      ...
      return [
        ...
        outline,
        '',
        'Respond in exactly this three-part format in order:',
        '1. ' + REPLY_MARK + ' <a short, warm reply (1-3 sentences, plain text) that names the technique you chose, what you did, and invites the next step. Natural language \u2014 like you\u2019re sitting next to them.>',
        '2. ' + ACTIONS_MARK + ' <a JSON array of actions to apply to the map, or [] for none. See allowed actions below.>',
        '3. If the conversation calls for a checklist of action items (e.g. tasks to do, steps to follow, items to consider), include a checklist section after the actions using markdown checkbox syntax. Use - [ ] for unchecked, - [x] for checked items. Only use checklists when the user explicitly asks for a plan, todo list, or step-by-step. Do NOT use checklists for brainstorming, ideas, or general discussion \u2014 those belong in @@ACTIONS@@ or @@REPLY@@.',
        '',
        'Examples of when to use checklists: the user says "give me a to-do list", "what are the next steps", "plan the launch", "list what I need to do".',
        'Examples of when NOT to use checklists: the user says "help me think about", "what are some ideas for", "explain how". Use @@ACTIONS@@ to add tree nodes instead.',
        '',
